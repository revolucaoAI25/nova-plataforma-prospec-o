...
// Catálogo de CNAEs para o seletor da busca CNPJ (Casa dos Dados).
// Código: 7 dígitos sem pontuação (ex: "6911701"). Portado de
// modules/cnaes.py do produto atual.
...
export function buscarCnaes(termo: string): Cnae[] {
  ...
  return CNAES.filter(
    (c) =>
      ...
      c.setor.toLowerCase().includes(t),
  );
}

export const CODIGO_PARA_DESC: Record<string, string> = Object.fromEntries(
  CNAES.map((c) => [c.codigo, c.descricao]),
);
