...
import { enrollEmailTargets, atualizarCampanhaEmail, perfilComEmailDisparoHabilitado } from "@/lib/email-dispatch-db";
import { buscarLeadsDaPesquisa } from "@/lib/db";

const bodySchema = z.object({
  searchId: z.string().uuid().optional(),
  leads: z.array(z.object({ nome: z.string().optional(), email: z.string() })).optional(),
});
...
export async function POST(
  ...
  let leads: Array<{ nome?: string | null; email?: string | null }> = [];
  if (parsed.data.searchId) {
    const rows = await buscarLeadsDaPesquisa(supabase, parsed.data.searchId);
    leads = rows.map((r) => ({ ...r, nome: r.nome, email: r.email }));
  ...
  const resultado = await enrollEmailTargets(supabase, id, leads);
  if (parsed.data.searchId) {
    await atualizarCampanhaEmail(supabase, id, { tipo_origem: "busca_existente", origem_search_id: parsed.data.searchId });
  }
