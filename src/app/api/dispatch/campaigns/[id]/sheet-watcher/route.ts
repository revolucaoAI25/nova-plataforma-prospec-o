...
import { criarSheetWatcher, atualizarCampanha } from "@/lib/dispatch-db";

const bodySchema = z.object({
  sheetId: z.string().min(1),
  abaNome: z.string().min(1),
  colunaTelefone: z.string().min(1),
...
export async function POST(
  ...
  const watcherId = await criarSheetWatcher(supabase, id, parsed.data);
  if (!watcherId) return NextResponse.json({ error: "Não foi possível configurar o monitoramento." }, { status: 500 });

  await atualizarCampanha(supabase, id, { tipo_origem: "sheet_watch" });
