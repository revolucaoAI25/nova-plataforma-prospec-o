...
import type { DispatchCampaignRow, DispatchTargetRow } from "@/lib/database.types";
...
export function ReportsPanel({ campanhasIniciais }: { campanhasIniciais: DispatchCampaignRow[] }) {
  const [campaignId, setCampaignId] = useState(campanhasIniciais[0]?.id ?? "");
  const [stats, setStats] = useState<Record<string, number> | null>(null);
  const [targets, setTargets] = useState<DispatchTargetRow[] | null>(null);
  ...
  useEffect(() => {
    ...
    // Busca de dados disparada por uma dependência (campaignId) mudando —
    // não dá pra "ajustar durante a renderização" aqui, é assíncrono.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setLoading(true);
    Promise.all([
      fetch(`/api/dispatch/campaigns/${campaignId}`).then((r) => r.json()),
      fetch(`/api/dispatch/campaigns/${campaignId}/targets`).then((r) => r.json()),
