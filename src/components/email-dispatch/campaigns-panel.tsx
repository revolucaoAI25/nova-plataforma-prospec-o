...
import type { EmailCampaignRow, EmailSenderRow } from "@/lib/database.types";
...
export function CampaignsPanel({
  ...
  campanhasIniciais: EmailCampaignRow[];
  ...
  async function criar(e: React.FormEvent) {
    e.preventDefault();
    if (!nome.trim() || !senderId) return;
    setCreating(true);
    setError(null);
    const resp = await fetch("/api/email-dispatch/campaigns", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nome, senderId, tipoOrigem: "manual" }),
    });
    ...
    setNome("");
    const listResp = await fetch("/api/email-dispatch/campaigns");
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <ul className="divide-y divide-border">
          {campanhas.map((c) => (
            <li key={c.id} className="flex items-center justify-between py-3">
              <div>
                <Link href={`/disparo-email/campanhas/${c.id}`} className="font-medium hover:underline">{c.nome}</Link>
                <p className="text-xs text-muted-foreground">
                  {c.intervalo_min_seg}–{c.intervalo_max_seg}s entre lotes
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant={STATUS_VARIANT[c.status] ?? "outline"}>{c.status}</Badge>
                <Button asChild variant="ghost" size="sm">
                  <Link href={`/disparo-email/campanhas/${c.id}`}>
        ...
        <form onSubmit={criar} className="flex flex-wrap gap-2">
          ...
          <Select value={senderId} onValueChange={setSenderId}>
            <SelectTrigger className="w-56"><SelectValue placeholder="Remetente" /></SelectTrigger>
            <SelectContent>
              {senders.map((s) => (
          ...
          <Button type="submit" disabled={creating || !senderId}>
