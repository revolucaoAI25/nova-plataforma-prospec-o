...
function SheetWatcherCard({ campaignId, watcherInicial }: { campaignId: string; watcherInicial: EmailSheetWatcherRow | null }) {
  const [watcher, setWatcher] = useState(watcherInicial);
  const [sheetUrl, setSheetUrl] = useState("");
  const [abaNome, setAbaNome] = useState("Leads");
  const [colunaEmail, setColunaEmail] = useState("E-mail");
  ...
  async function configurar(e: React.FormEvent) {
    ...
    if (!sheetId || !abaNome.trim() || !colunaEmail.trim()) return;
    setSaving(true);
    setError(null);
    const resp = await fetch(`/api/email-dispatch/campaigns/${campaignId}/sheet-watcher`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sheetId, abaNome, colunaEmail, colunaNome: colunaNome || undefined }),
    });
        ...
        {watcher ? (
          <div className="flex flex-col gap-2 rounded-xl border border-primary/20 bg-accent p-3 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-accent-foreground">
                Aba <strong>{watcher.aba_nome}</strong>, coluna e-mail <strong>{watcher.coluna_email}</strong>
          ...
          <form onSubmit={configurar} className="flex flex-col gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="watcher-col-email">Coluna e-mail</Label>
                <Input id="watcher-col-email" value={colunaEmail} onChange={(e) => setColunaEmail(e.target.value)} placeholder="E-mail ou B" />
              </div>
              <div className="flex flex-col gap-1.5">
...
export function CampaignDetail({
  ...
  campanha: EmailCampaignRow;
  etapasIniciais: EmailCadenceStepRow[];
  statsIniciais: Record<string, number>;
  senders: EmailSenderRow[];
  pesquisas: SearchRow[];
  targetsIniciais: EmailTargetRow[];
  watcherInicial: EmailSheetWatcherRow | null;
  ...
  async function removerAlvo(targetId: string) {
    if (!confirm("Remover este alvo? Ele para de receber e-mails desta e de qualquer outra campanha sua (opt-out).")) return;
    setRemovendoId(targetId);
    const resp = await fetch(`/api/email-dispatch/campaigns/${campanha.id}/targets/${targetId}`, { method: "DELETE" });
    setRemovendoId(null);
    if (resp.ok) {
  ...
  const sender = senders.find((s) => s.id === campanha.sender_id);

  async function alternarStatus() {
    const novo = status === "ativa" ? "pausada" : "ativa";
    const resp = await fetch(`/api/email-dispatch/campaigns/${campanha.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function adicionarEtapa(e: React.FormEvent) {
    ...
    setAddingStep(true);
    const resp = await fetch(`/api/email-dispatch/campaigns/${campanha.id}/steps`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        assunto,
        corpo,
        templateId: templateOrigemId || undefined,
      }),
    });
    setAddingStep(false);
    if (resp.ok) {
      ...
      setAtrasoHoras(0);
      router.refresh();
      const listResp = await fetch(`/api/email-dispatch/campaigns/${campanha.id}`);
  ...
  async function removerEtapa(stepId: string) {
    await fetch(`/api/email-dispatch/campaigns/${campanha.id}/steps/${stepId}`, { method: "DELETE" });
  ...
  async function inscrever() {
    ...
    if (searchId) {
      ...
      body.leads = manualLista
        ...
        .filter(Boolean)
        .map((l) => {
          const [nome, email] = l.split(",").map((x) => x.trim());
          return email ? { nome, email } : { nome: "", email: nome };
        });
    } else {
      setFeedback({ ok: false, msg: "Escolha uma pesquisa do histórico ou cole uma lista de nome,e-mail." });
      setEnrolling(false);
      return;
    }

    const resp = await fetch(`/api/email-dispatch/campaigns/${campanha.id}/enroll`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    setSearchId("");
    router.refresh();
    const statsResp = await fetch(`/api/email-dispatch/campaigns/${campanha.id}`);
    ...
    <div className="flex flex-col gap-6">
      <Card>
        <CardHeader className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <CardDescription>Remetente: {sender ? `${sender.from_name} <${sender.from_email}>` : "—"}</CardDescription>
          </div>
          <div className="flex items-center gap-2">
        ...
        <CardContent className="flex flex-col gap-4">
            ...
            <div key={e.id} className="flex items-start justify-between gap-3 rounded-xl border border-border p-3">
              <div className="min-w-0">
                <p className="text-xs text-muted-foreground">
                  Etapa {e.ordem} · {e.atraso_horas}h de atraso
                </p>
                <p className="text-sm font-medium">{e.assunto}</p>
                <p className="whitespace-pre-wrap text-sm text-muted-foreground">{e.corpo}</p>
              </div>
          ...
          <form onSubmit={adicionarEtapa} className="flex flex-col gap-3 rounded-xl border border-dashed border-border p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {templates.length > 0 && (
                <div className="ml-auto flex items-center gap-2">
                  <Select value={templateOrigemId} onValueChange={carregarTemplate}>
                    <SelectTrigger className="h-9 w-56"><SelectValue placeholder="Carregar de um template" /></SelectTrigger>
                    <SelectContent>
                      {templates.map((t) => (
                        <SelectItem key={t.id} value={t.id}><FileText className="h-3.5 w-3.5" /> {t.nome}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input id="assunto" value={assunto} onChange={(e) => setAssunto(e.target.value)} placeholder="Use {{nome}} para inserir o nome do lead" />
            </div>
            <div className="flex flex-col gap-1.5">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>De uma pesquisa do histórico, ou colando uma lista (uma linha por lead: nome,e-mail).</CardDescription>
        </CardHeader>
